...
print("[INFO] Initializing Face Detector and 68-Landmark Predictor...")
detector = dlib.get_frontal_face_detector()
try:
    predictor = dlib.shape_predictor(PREDICTOR_PATH)
except Exception as e:
    print(f"[ERROR] Could not load model from {PREDICTOR_PATH}. Check file path!")
    exit(1)

print(f"[INFO] Opening Camera: {CAMERA_PATH}")
cap = cv2.VideoCapture(CAMERA_PATH)

if not cap.isOpened():
    print("[WARN] Target device path failed, falling back to default camera index 0...")
    cap = cv2.VideoCapture(0)

cv2.namedWindow("Behavioral AI Check - Risk Analysis", cv2.WINDOW_NORMAL)
...
while True:
    ...
    cv2.imshow("Behavioral AI Check - Risk Analysis", frame)
